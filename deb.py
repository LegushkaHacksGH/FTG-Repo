...
class DebMod(loader.Module):
  ...
  async def debcmd(self, m):
    ...
    if not image:
      await m.edit("ты еблан блять? .deb <реплай на пикчу или стикер>")
      return
    await m.edit("Дэб'им...")
    deb = Image.open(io.BytesIO(get("https://raw.githubusercontent.com/LegushkaHacksGH/FTG-Stuff/main/deb_PNG.png").content))
